...
VALGRIND_ENABLE_ERROR_REPORTING;
		...
		if (originating_server_hostname == current_discovered_hostname) {
			if (current_discovered_read_only == 0) {
				AWS_RDS_Topology_Server_Map[originating_server_hostname]->add_writer_hostgroup(writer_hostgroup);
			} else {
				AWS_RDS_Topology_Server_Map[originating_server_hostname]->writer_hostgroups.clear();
				AWS_RDS_Topology_Server_Map[originating_server_hostname]->add_reader_hostgroup(reader_hostgroup);
			}
		}
		...
		if (!AWS_RDS_Topology_Server_Map.count(current_discovered_hostname)) {
			...
			new_servers.push_back(discovered_server);
			auto new_rds_topology_server = make_shared<AWS_RDS_topology_server>(current_discovered_hostname, current_discovered_port);
			new_rds_topology_server->add_reader_hostgroup(reader_hostgroup);
